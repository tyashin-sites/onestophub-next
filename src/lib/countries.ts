...
export const ALL_COUNTRY_OPTIONS: CountryOption[] = Object.entries(COUNTRY_NAMES)
...
export function normalizeCountryCode(country: string): CountryCode | null {
  const normalizedValue = country.trim();
  if (!normalizedValue) return null;

  const upperCode = normalizedValue.toUpperCase() as CountryCode;
  if (upperCode in COUNTRY_NAMES) {
    return upperCode;
  }

  return COUNTRY_NAME_TO_CODE[normalizedValue.toLowerCase()] ?? null;
}

export function getCountryOptions(countryCodes?: string[]): CountryOption[] {
  ...
  return countryCodes
    ...
    .map((countryCode) => ({ code: countryCode, name: COUNTRY_NAMES[countryCode] }))
    .sort((left, right) => left.name.localeCompare(right.name));
}